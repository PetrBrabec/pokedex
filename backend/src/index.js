	...
	Query: {
		pokemons: (__, args) => {
			...
			if (filter) {
				...
				if (filter.weakness) {
					const regex = new RegExp(filter.weakness, 'i');
					pokemons = _.filter(pokemons, (p) =>
						_.some(p.weaknesses, (t) => t.match(regex)),
					);
				}

				if (filter.resistance) {
					const regex = new RegExp(filter.resistance, 'i');
					pokemons = _.filter(pokemons, (p) =>
						_.some(p.resistant, (t) => t.match(regex)),
					);
				}
